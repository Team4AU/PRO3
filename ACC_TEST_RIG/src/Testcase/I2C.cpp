...
I2C::readRegisters(unsigned int length, unsigned int FromAddress) {
    ...
    if(::write(this->file,buffer,1) != 1) {
    	//throw here
    	throw busException("I2C: Failed write to the device\n",1);
    ...
    if(::read(this->file, data, length)!=(int)length){
    	//throw here
    	throw busException("IC2: Failed to read in the full buffer.\n",2);
...
int I2C::open() {
    ...
    if((this->file=::open(I2Cname.c_str(), O_RDWR)) < 0){
    	//throw here
    	throw busException("I2C: failed to open the bus\n",4);
        //perror("I2C: failed to open the bus\n");
        return -1;
    }
    if(ioctl(this->file, I2C_SLAVE, this->deviceId) < 0){
    	//throw here
    	throw busException("I2C: Failed to connect to the address\n",5);
