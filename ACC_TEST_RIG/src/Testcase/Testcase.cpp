...
{
	//try to free so it can be used again
	if(this->dataP != NULL){
		free(this->dataP);
	}
	...
	{
		this->dataP = new dataPoint[SAMPLES_I2C_ADXL];
		this->dataLength = SAMPLES_I2C_ADXL;
	...
	{
		this->dataP = new dataPoint[SAMPLES_SPI_ADXL];
		this->dataLength = SAMPLES_SPI_ADXL;
	...
	{
		this->dataP = new dataPoint[SAMPLES_I2C_KX224];
		this->dataLength = SAMPLES_I2C_KX224;
		...
		if ((code = (this->initpwm (pwm))) != 0)
		{
			std::cout << "KXX + SPI - 3" << std::endl;
			return code;
		}
		if (!pwm.startPWM ())
		{
			std::cout << "KXX + SPI - 4" << std::endl;
			return TestStatusCodes::_514;
		}
		if (kx224.ReadSensorState (this->dataP,SAMPLES_I2C_KX224) != 0)
		{
			std::cout << "KXX + SPI - 5" << std::endl;
			return TestStatusCodes::_520;
		}
		if (!pwm.stopPWM ())
		{
			std::cout << "KXX + SPI - 6" << std::endl;
			return TestStatusCodes::_515;
		}
	...
	{
		this->dataP = new dataPoint[SAMPLES_SPI_KX224];
		this->dataLength = SAMPLES_SPI_KX224;
